...
def _get_utt_split_lists(
    ...
    for data_folder in data_folders:
        ...
        test_spks = [snt.split(os.path.sep)[0] for snt in test_lst]
        ...
        if split_speaker:
            ...
            for f in glob.glob(path, recursive=True):
                spk_id = f.split(os.path.sep+"wav"+os.path.sep)[1].split(os.path.sep)[0]
...
def prepare_csv(seg_dur, wav_lst, csv_file, random_segment=False, amp_th=0):
    ...
    for wav_file in tqdm(wav_lst, dynamic_ncols=True):
        ...
        try:
            [spk_id, sess_id, utt_id] = wav_file.split(os.path.sep)[-3:]
        except ValueError:
...
def prepare_csv_enrol_test(data_folders, save_folder, verification_pairs_file):
    ...
    for data_folder in data_folders:
        ...
        enrol_csv = []
        for id in enrol_ids:
            wav = data_folder + os.path.sep+"wav" +os.path.sep+ id + ".wav"
            ...
            start_sample = 0
            stop_sample = signal.shape[0]
            [spk_id, sess_id, utt_id] = wav.split(os.path.sep)[-3:]
